                ...
                if not check_location(box_y, box_height, height):
                    ...
                    if check_new_object and check_start_line(box_y, box_height):
                        ...
                        draw_prediction(CLASSES, colors, frame, new_object['id'], new_object['confidence'],
                                        box_x, box_y, box_width, box_height)
        # Put summary text
        cv2.putText(frame, "Total : {:03d}".format(number_vehicle), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (247, 224, 20), 2)
        # label = str(classes[class_id])
        cv2.putText(frame, "Car  : {:03d}".format(number_car), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), 2)
        cv2.putText(frame, "Bus  : {:03d}".format(number_bus), (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), 2)
        cv2.putText(frame, "Truck  : {:03d}".format(number_truck), (20, 78), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), 2)
        # Draw start line
